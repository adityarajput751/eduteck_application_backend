...
const addAdmin = async (req, res) => {
  try {
    ...
    if (!name || !email || !password) {
      res.json({ success: false, message: 'Please add all details' })
    }
    ...
    if (existingAdmin) {
      ...
      const adminData = {
        name, email, password: hasedPassword, created_on
      }
      const newAdmin = new adminModel(adminData);
      await newAdmin.save();
      res.json({ success: true, message: "Admin has been added successfully." });
    }
  } catch (error) {
    console.log(error);
